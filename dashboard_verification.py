...
MARKET_OPEN_HOUR = 9   # 9h00 (UTC)
MARKET_CLOSE_HOUR = 21  # 21h00 (UTC)
...
def check_binance():
    try:
        ...
        return {"status": f"❌ Binance API Error: {e}, bascule vers CoinGecko"}

# Vérification de CoinGecko API
def check_coingecko():
    cached_data = get_cached_response("coingecko")
    if cached_data:
        return cached_data
    try:
        ...
        data = response.json()
        set_cached_response("coingecko", {"status": "✅ CoinGecko fonctionne", "prices": data})
...
def check_yahoo():
    try:
        ...
        if response.status_code != 200:
            return {"status": f"❌ Yahoo Finance API Error: HTTP {response.status_code}"}
        data = response.json()
...
def check_indices():
    try:
        ...
        if response.status_code != 200:
            return {"status": f"❌ Indices API Error: HTTP {response.status_code}"}
        data = response.json()
...
if is_market_time():
    ...
    verification_results = {"status": "⏳ Données non mises à jour - Prochaine mise à jour à l'ouverture ou à la fermeture du marché."}
